 ...
 */
public class Hw1SwitchTest7 extends FloodlightTestCase {
    ...
    @Test
    public void testTwoSwitchBlock() throws Exception {
        // Build our test packet
      ...
      OFPacketOut po = new OFPacketOut()
        ...
        expect(mockSwitch2.getId()).andReturn(1L).anyTimes();

        int nPackets = 15 * 1024 * 1024 / newPacketIn.getTotalLength(); // expecting =15MB

        mockSwitch1.write(po, null);
        expectLastCall().times(nPackets - 1, nPackets + 1);
        mockSwitch2.write(po, null);
        expectLastCall().times(nPackets - 1, nPackets + 1);
        ...
        for (; nPackets >= 0; nPackets--) {
            listener.receive(mockSwitch1, newPacketIn, parseAndAnnotate(this.packetIn));
            listener.receive(mockSwitch2, newPacketIn, parseAndAnnotate(this.packetIn));
        }
