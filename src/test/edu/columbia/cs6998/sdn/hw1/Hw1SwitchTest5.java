 ...
 */
public class Hw1SwitchTest5 extends FloodlightTestCase {
    ...
    @Test
    public void testTwoSwitchesFlowMod() throws Exception {
        ...
        OFMessage fm1 = ((OFFlowMod) mockFloodlightProvider.getOFMessageFactory().getMessage(OFType.FLOW_MOD))
            .setActions(Arrays.asList(new OFAction[] {
                    new OFActionOutput().setPort((short) 10).setMaxLength((short) -1)}))
            .setBufferId(50)
        ...
        OFMessage fm2 = ((OFFlowMod) mockFloodlightProvider.getOFMessageFactory().getMessage(OFType.FLOW_MOD))
            .setActions(Arrays.asList(new OFAction[] {
                    new OFActionOutput().setPort((short) 20).setMaxLength((short) -1)}))
            .setBufferId(50)
            .setCommand(OFFlowMod.OFPFC_ADD)
            .setHardTimeout((short) Hw1Switch.HARD_TIMEOUT_DEFAULT)
            .setIdleTimeout((short) Hw1Switch.IDLE_TIMEOUT_DEFAULT)
            .setMatch(new OFMatch()
                .loadFromPacket(testPacketReplySerialized, (short) 3)
                .setWildcards(OFMatch.OFPFW_NW_PROTO | OFMatch.OFPFW_TP_SRC | OFMatch.OFPFW_TP_DST
                        | OFMatch.OFPFW_NW_TOS))
            .setOutPort(OFPort.OFPP_NONE.getValue())
            .setCookie(Hw1Switch.HW1_SWITCH_COOKIE)
        ...
        expect(mockSwitch1.getAttribute(IOFSwitch.PROP_FASTWILDCARDS)).andReturn((Integer) (OFMatch.OFPFW_IN_PORT | OFMatch.OFPFW_NW_PROTO
                | OFMatch.OFPFW_TP_SRC | OFMatch.OFPFW_TP_DST | OFMatch.OFPFW_NW_SRC_ALL
                | OFMatch.OFPFW_NW_DST_ALL | OFMatch.OFPFW_NW_TOS));
        mockSwitch1.write(fm1, null);

        expect(mockSwitch2.getAttribute(IOFSwitch.PROP_FASTWILDCARDS)).andReturn((Integer) (OFMatch.OFPFW_IN_PORT | OFMatch.OFPFW_NW_PROTO
                | OFMatch.OFPFW_TP_SRC | OFMatch.OFPFW_TP_DST | OFMatch.OFPFW_NW_SRC_ALL
                | OFMatch.OFPFW_NW_DST_ALL | OFMatch.OFPFW_NW_TOS));
        mockSwitch2.write(fm2, null);
        ...
        this.packetInReply.setBufferId(50);

        // Get the listener and trigger the packet in
        IOFMessageListener listener = mockFloodlightProvider.getListeners().get(
                OFType.PACKET_IN).get(0);
        this.packetIn.setInPort((short) 10);
        listener.receive(mockSwitch1, this.packetIn, parseAndAnnotate(this.packetIn));
        this.packetIn.setInPort((short) 20);
        listener.receive(mockSwitch2, this.packetIn, parseAndAnnotate(this.packetIn));

        this.packetInReply.setInPort((short) 2);
        listener.receive(mockSwitch1, this.packetInReply, parseAndAnnotate(this.packetInReply));
        this.packetInReply.setInPort((short) 3);
        listener.receive(mockSwitch2, this.packetInReply, parseAndAnnotate(this.packetInReply));
